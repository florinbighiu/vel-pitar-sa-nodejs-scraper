...
## Project
VEL PITAR SA scraper for peviitor.ro (Node.js, ESM, Jest)

## 🌱 This Repo Is a Derived Scraper
This repo is a **derived scraper** built from the [EPAM template](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper). It follows the same architecture and conventions.
...
- **Only the API parsing logic in `scraper/index.js`** (`fetchJobsPage`, `parseApiJobs`) is company-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo florinbighiu/vel-pitar-sa-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo florinbighiu/vel-pitar-sa-nodejs-scraper --state open`
