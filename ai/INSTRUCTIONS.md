...
This scraper extracts job listings from the velpitar.ro WordPress REST API and imports them to peviitor.ro.

Target: https://velpitar.ro/cariere-vel-pitar/
...
1. **Start with brand** - We know the brand ("Vel Pitar")
...
8. **Scrape new jobs** - Extract jobs from the velpitar.ro WP REST API (category "Cariere")
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts VEL PITAR jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from the velpitar.ro WP REST API (category "Cariere")
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape velpitar.ro WP REST API (category "Cariere", city categories → locations)
    │
    ▼
...
- **VEL PITAR WP REST API**: `https://velpitar.ro/wp-json/wp/v2/posts` — GET with `categories` (career category id), `per_page`, `page`
